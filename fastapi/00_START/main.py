...
from typing import List, Dict, Tuple, Set, Any, Optional, Union  # typing 모듈 List, Dict, Tuple, Set, Any, Optional, Union 기본 제공 데이터 타입 가져옴
...
@app.get("/databoolitems/")
# 고급 타입 힌트(Type Hint) bool 자료형 매개변수 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 bool 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 bool이다. (true 또는 false)
# 웹브라우저 테스트
# http://127.0.0.1:8000/databoolitems/?data=true -> 결과 true
# http://127.0.0.1:8000/databoolitems/?data=false -> 결과 false
async def read_items(data: bool):  # GET 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) bool 형식
    return data  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환

@app.get("/dataitems/")
# 고급 타입 힌트(Type Hint) float 자료형 매개변수 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 float 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 float이다. (정수 및 소수점 포함)
# 웹브라우저 테스트
# http://127.0.0.1:8000/dataitems/?data=1 -> 결과 1 (정수 및 소수점 포함)
# http://127.0.0.1:8000/dataitems/?data=1.1 -> 결과 1.1 (정수 및 소수점 포함)
# http://127.0.0.1:8000/dataitems/?data=fun -> 오류 data=fun float 아님
async def read_items(data: float):  # GET 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) float 형식
    return data  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환
...
@app.post("/create-tupleitem/") # HTTP POST 요청 경로 지정
# 고급 타입 힌트(Type Hint) Tuple[str, int] 자료형 요청 본문 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 Tuple[str, int] 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 Tuple이고, Tuple 각각의 값의 쌍은 [str, int]여야 한다.
# 다만 고급 타입 힌트(Type Hint)가 되려면 Tuple[str, int] 이렇게 작성해야 한다.
async def create_item(item: Tuple[str, int]):  # POST 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) Tuple[str, int] 형식
    return item  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환

@app.post("/create-setitem/") # HTTP POST 요청 경로 지정
# 고급 타입 힌트(Type Hint) Set[str, int] 자료형 요청 본문 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 Set[str, int] 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 Set이고, Set 각각의 값의 쌍은 [str, int]여야 한다.
# 다만 고급 타입 힌트(Type Hint)가 되려면 Set[str, int] 이렇게 작성해야 한다.
async def create_item(item: Set[bool]):  # POST 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) Set[str, int] 형식
    return item  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환

@app.post("/create-noneitem/") # HTTP POST 요청 경로 지정
# 고급 타입 힌트(Type Hint) None 자료형 요청 본문 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 None 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 None이고, 값을 넣으면 안 된다. (값 자체를 전달할 수 없음.)
# 다만 고급 타입 힌트(Type Hint)가 되려면 None 이렇게 작성해야 한다.
# 웹브라우저 테스트
# http://127.0.0.1:8000/create-noneitem/?item=1 -> 오류 item=1 None 아님
async def create_item(item: None):  # POST 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) None 형식
    return item  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환

@app.post("/create-anyitem/") # HTTP POST 요청 경로 지정
# 고급 타입 힌트(Type Hint) Any 자료형 요청 본문 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 Any 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 Any이고, 모든 타입 허용, 타입 검사 무시 의미
# 다만 고급 타입 힌트(Type Hint)가 되려면 Any 이렇게 작성해야 한다.
# 웹브라우저 테스트
# http://127.0.0.1:8000/create-anyitem/?item=1 -> 결과 "1"
# http://127.0.0.1:8000/create-anyitem/?item=fun -> 결과 "fun"
async def create_item(item: Any):  # POST 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) Any 형식
    return item  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환

@app.post("/create-optional-item/") # HTTP POST 요청 경로 지정
# 고급 타입 힌트(Type Hint) Optional[int] = None 자료형 요청 본문 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 Optional[int] = None 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 Optional[int] = None이고, int 또는 None 의미
# 다만 고급 타입 힌트(Type Hint)가 되려면 Optional[int] = None 이렇게 작성해야 한다.
# 주의사항: Optional[int] 이것만 쓰면 오류 발생
# 웹브라우저 테스트
# http://127.0.0.1:8000/create-optional-item/ -> 결과 "null"
# http://127.0.0.1:8000/create-optional-item/?item=1 -> 결과 1
# http://127.0.0.1:8000/create-optional-item/?item=fun -> 오류 item=fun int 아님
async def create_item(item: Optional[int] = None):  # POST 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) Optional[int] = None 형식
    return item  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환

@app.post("/create-union-item/") # HTTP POST 요청 경로 지정
# 고급 타입 힌트(Type Hint) Union[bool, int] 자료형 요청 본문 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 Union[bool, int] 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 Union[bool, int]이고, bool 또는 int 의미
# 다만 고급 타입 힌트(Type Hint)가 되려면 Union[bool, int] 이렇게 작성해야 한다.
# 웹브라우저 테스트
# http://127.0.0.1:8000/create-union-item/?item=0 -> 결과 false
# http://127.0.0.1:8000/create-union-item/?item=1 -> 결과 true
# http://127.0.0.1:8000/create-union-item/?item=2 -> 결과 2
# http://127.0.0.1:8000/create-union-item/?item=fun -> 오류 item=fun int 또는 bool 아님
async def create_item(item: Union[bool, int]):  # POST 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) Union[bool, int] 형식
    return item  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환


@app.post("/create-list-dict-union-item/") # HTTP POST 요청 경로 지정
# 고급 타입 힌트(Type Hint) List[Dict[str, Union[int, str]]] 자료형 요청 본문 사용 시 
# 복잡한 데이터 구조 표현 가능 및 아래처럼 작성된 List[Dict[str, Union[int, str]]] 데이터 타입 이외의 이상한 값들이 들어오는 것을 막을 수 있다.
# 데이터 타입 List[Dict[str, Union[int, str]]]이고, 문자열 키와 정수 또는 문자열 값이 있는 딕셔너리의 리스트 의미
# 다만 고급 타입 힌트(Type Hint)가 되려면 List[Dict[str, Union[int, str]]] 이렇게 작성해야 한다.
# 웹브라우저 테스트
# http://127.0.0.1:8000/create-list-dict-union-item/
# -> 입력(Request body)
# [
#   {
#     "dave": 0,
...
#   }
# ]
async def create_item(item: List[Dict[str, Union[int, str]]]):  # POST 요청 처리 async 비동기 함수 - 고급 타입 힌트(Type Hint) List[Dict[str, Union[int, str]]] 형식
    return item  # JSON 형태 응답 반환(구글 크롬 웹브라우저). FastAPI가 자동 변환
...
# {"skip":0,"limit":10}
# C:\Users\NT371B5A>curl "http://127.0.0.1:8000/databoolitems/?data=true"
# true
# C:\Users\NT371B5A>curl "http://127.0.0.1:8000/databoolitems/?data=false"
# false
# C:\Users\NT371B5A>curl "http://127.0.0.1:8000/databoolitems/?data=fun"
# {"detail":[{"type":"bool_parsing","loc":["query","data"],"msg":"Input should be a valid boolean, unable to interpret input","input":"fun","url":"https://errors.pydantic.dev/2.13/v/bool_parsing"}]}
# C:\Users\NT371B5A>
